...
# ╔═╡ 95faa990-eea8-11ec-010e-8b2d762a50d3
using DifferentialEquations

# ╔═╡ eddcff77-0e56-43cc-8a9a-58bd390dfbf0
using Plots

# ╔═╡ 4049035d-7967-4631-9a6e-50200d657d2f
# hideall
title = "EDOs y Emojis";


# ╔═╡ c9eb7f40-b927-4f0c-9b3a-f4b067a3a7c4
"""
+++
title = "$title"
+++
...
# ╔═╡ 9c3a5a4a-db76-4cd0-ba35-2277224cbbf6
md"
### Sistema depresador-presa
Consideremos la situacion donde tenemos una especie, llamada presa, la cual tiene un suministro amplio de alimento y la segunda, llamada depredador se alimante de la primera. Un ejemplo de esto podrían ser conejos y lobos, peces y tiburones, etc. 
El modelo tiene dos variables dependientes y ambas que son funciones de tiempo. $R(t)$ representa a los conejos (rabbit) y $W(t)$ a los lobos (wolf)
En ausencia de depredadores, el suministro amplio de alimento apoyaria al crecimiento de la prese, de manera exponencial, es decir:

$$\frac{dR}{dt} = k R$$ donde r es una constante positiva

En ausencia de la presa, se supone que la poblacion de depradores disminuiría con una rapidez proporcional a sí misma, es decir

$\frac{dW}{dt} = -r W$
donde r es una constante positiva

Sin embargo, con ambas especies presentes, se supone que la causa principal de muerte entre la presa que está siendo comida por un depredador y los ritmos de natalidad y supervivencia de los depredadores depende de su suministro de alimento variable, es decir, la presa. Se supone también que las dos especies se encuentran entre sí con una frecuencia que es proporcional a ambas poblaciones y, por tanto, es proporcional al producto $RW$.
(Mientras mayor sea la cantidad de cualquier población, es más probable que haya mayor número de encuentros.) Un sistema de dos ecuaciones diferenciales que incorpora estas suposiciones, es como sigue
...
# ╔═╡ 0623a78e-493a-4c99-adb8-030f5ea6fb2d
begin
	function latka_voterra!(du,u,o,t)
		🐰, 🐺 = u
		α,β,γ,δ = p
		du[1]= d🐰 = α*🐰 -  β*🐰*🐺
		du[2] = d🐺 = γ*🐰*🐺 - δ*🐺
	...
	tspan = (0.0,10.0)
	p = [1.5,1.0,3.0,1.0]
	prob = ODEProblem(latka_voterra!,u₀,tspan)
	sol = solve(prob)

end

# ╔═╡ 63a75000-20ed-47ba-b6cd-240c7b18b8c3
plot(sol)

# ╔═╡ 808ac57d-f0ee-4440-a37b-364b4c641316
plot(sol,vars=(1,2))
